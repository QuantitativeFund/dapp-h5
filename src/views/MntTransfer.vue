<template>
    <div class="box">
        <div class="title">
            ...
            <div class="title-text">{{ t('Common.MNT_transfer') }}</div>
        </div>
        <div class="mnt-balance">
            <div class="mnt-balanceText">{{ t('Common.MNT_balance') }}</div>
            <div class="mnt-balanceContent">{{ BigNumber(user.MNT).toFixed(6) }}</div>
        </div>
        <div class="transfer-quantity">{{ t('Common.MNT_recharge_amount') }}</div>
        <div class="input-mnt-box">
            <van-field type="number" v-model="mnt_v" :placeholder="t('Common.transfer_v')" autocomplete="off"
        ...
        <van-field maxlength="42" type="textarea" v-model="mnt_addr"
        ...
        <van-button class="mnt-btn" @click="mnt_transfer">MNT {{ t("Common.transfer") }}
        </van-button>
    </div>
...
function mnt_transfer() {
    if (mnt_prompt()) {
        return;
    }
    if (!isAddress(mnt_addr.value)) {
        //showFailToast("朋友地址错误");
        showFailToast(t('Common.address_error'));
        return;
    }
    if (mnt_v.value == "") {
        //showFailToast("请填写MNT数量");
        showFailToast(t('Common.transfer_v_placeholder'));
        return;
    }
    if (parseEther(mnt_v.value) > parseEther(user.MNT)) {
    ...
    op = async () => {
        if (password.value == user.password) {
            ...
            mnt_addr.value = "";
            mnt_v.value = "";
            showFailToast(t("Common.transfer_success"));
