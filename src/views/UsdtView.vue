<template>
  <div class="box">
    <div class="title">
      ...
      <div class="title-content">{{ t('UsdtMnt.USDT_running_water') }}</div>
    </div>
    <van-pull-refresh v-model="loading" @refresh="load" style="min-height: 90vh" v-show="usdt.length != 0">
      <van-cell-group v-for="(obj, key) in usdt" :key="key" class="mnt-stream" inset>
        <van-cell title="USDT"
    ...
    <van-empty :description="t('UsdtMnt.data_empty')" v-show="usdt.length == 0" />
  </div>
</template>
